...
export default function DonorList() {
  ...
  const fetchDonors = async () => {
    setLoading(true);
    const { data, error } = await supabase.from('donor').select('*').order('created_at', { ascending: false });
    if (error) {
      ...
      .on(
        ...
        (payload) => {
          ...
          switch (payload.eventType) {
            ...
            case 'UPDATE':
              setDonors(current =>
                current.map(d => d.id === payload.new.id ? payload.new as Donor : d)
              );
              break;
            case 'DELETE':
              setDonors(current =>
  ...
  if (error) {
    return (
      <div className="text-red-500 text-center mt-4">
        ...
        <button onClick={fetchDonors} className="mt-2 px-4 py-2 bg-blue-600 text-white rounded">Retry</button>
      </div>
    );
  }
    ...
    <div className="mt-6">
      ...
      {loading && donors.length === 0 ? (
        ...
        <div className="space-y-3">
          {donors.map(donor => (
            <div key={donor.id} className="p-4 border border-gray-200 rounded-lg bg-white shadow-sm hover:shadow-md">
              <div className="flex flex-col justify-between items-start">
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-gray-600">Medium: {donor.medium_name} ({donor.medium_number})</p>
                  <p className="text-sm text-gray-600">Address: {donor.address}</p>
                  <p className="text-sm text-gray-600">Donor ID: <span className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded">{donor.donor_id}</span> </p>
                  {donor.created_at && (
