...
export default function DonationDisplayPage() {
  ...
  return (
    <div className="flex flex-col items-center mt-10">
      <h2 className="text-2xl font-bold mb-4">Donations ({selectedYear})</h2>
      <div className="flex justify-between items-center mb-4">
        <div className="flex flex-row justify-between gap-2 min-w-2xl">
          <select
            value={selectedYear}
          ...
          <input
            ...
            className="px-3 min-w-xl py-2 border border-green-300 rounded-md focus:outline-none focus:ring focus:border-green-600"
          />
        </div>
      </div>
      ...
      {loading && donors.length === 0 ? (
        ...
        <div className="space-y-4 min-w-2xl">
            <div className="flex justify-between items-center mb-4">
  <h2 className="text-2xl font-bold">Donor List ({filteredDonors.length})</h2>
    ...
    <p className="text-lg font-bold text-green-700">
      ৳ {totalYearlyDonation.toLocaleString()}
          ...
          {filteredDonors.map((donor) => {
            ...
            const totalDonation = donorDonations.reduce(
              (sum, d) => sum + (d.amount ?? 0),
              0
            );
            ...
            return (
              <div
                key={donor.id}
                className="p-4 mb-4 border border-gray-200 rounded-lg bg-white shadow-sm hover:shadow-md"
              >
